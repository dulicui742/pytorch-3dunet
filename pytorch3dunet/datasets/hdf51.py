...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path, phase, slice_builder_config, transformer_config, raw_internal_path='raw',
        ...
        # self.raw[self.raw > max_value] = max_value
        # self.raw[self.raw < min_value] = max_value
        ...
        print(f"slice time: {time.time() - start_time:.2f} seconds")
    ...
    def create_datasets(cls, dataset_config, phase):
        ...
        if phase == "train":
            num = 200 #int(num_tmp * 2/3) ##300
        elif phase == "val":
            num = 50 #if int(num_tmp * 1/3) else int(num_tmp * 1/3)
        else:
            num = len(file_paths)
        file_paths1 = random.sample(file_paths, num)
        ### ==============MEM is not enough==========
        ...
        for cnt, file_path in enumerate(file_paths1):
            if phase == "test":
                # import pdb; pdb.set_trace()
                uid = int(file_path.split("/")[-1].split(".")[0])
                # if uid not in range(1,11): #["17", "98"]:#["42", "16"]:
                if uid not in [1]: #### debug ??????????????
                    continue
            try:
                logger.info(f'Loading sample-{cnt} {phase} set from: {file_path}...')
...
class THStandardHDF5Dataset2(AbstractHDF5Dataset):
    """
